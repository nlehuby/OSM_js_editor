...
function init_form_from_OSM(form, OSM_id) {
    ...
    name_src = get_tag(OSM_xml, "name");
    if (name_src == "undefined") {
        name_src = ""
    };
    opening_src = get_tag(OSM_xml, "opening_hours");
    if (opening_src == "undefined") {
        opening_src = ""
    };
    ...
    for (i = 0; i < n; i++) {
        ...
        if ((inputForm[i].type.toLowerCase() === "radio") && (inputForm[i].name === "wifi")) {
            if ((wifi_src === "wlan") || (wifi_src === "yes")) {
                if (inputForm[i].value === "wlan") {
                    inputForm[i].checked = true
                }
            }
            if (wifi_src === "undefined") {
                if (inputForm[i].value === "unknown") {
                    inputForm[i].checked = true
                }
            }
            if (wifi_src === "no") {
                if (inputForm[i].value === "no") {
                    inputForm[i].checked = true
                }
            }
        }
...
function form_from_user(form) {
    ...
    if (wifi != get_tag(OSM_xml, "internet_access")) {
        if (wifi == "unknown") {
            del_tag(OSM_xml, "internet_access");
            envoi = 4
        } else {
            edit_tag(OSM_xml, "node", "internet_access", wifi);
            envoi = 3
        }
    }
    if ((name != get_tag(OSM_xml, "name")) && (name != "")) {
        edit_tag(OSM_xml, "node", "name", name);
        envoi = 5
    }
    //si nom est vide mais qu'avant il y avait qqch, il faut supprimer le tag
    else {
        if ((get_tag(OSM_xml, "name") != "non_fourni") && (name == "")) {
